...
const evidenceBadge = evidence => evidence ? `<small class="field-evidence">${Math.round(Number(evidence.confidence || 0) * 100)}% · page ${escapeHtml(evidence.page || '?')} · ${escapeHtml(evidence.source || 'unknown source')}</small>` : '<small class="field-evidence uncertain">Manual verification required</small>';
