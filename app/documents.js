...
async function openDocument(id) {
  ...
  try {
    ...
    const inputs = [['invoiceNumber', 'Invoice number', fields.invoiceNumber], ['invoiceDate', 'Invoice date', fields.invoiceDate], ['vendorName', 'Vendor', fields.vendor?.name], ['gstin', 'Vendor GSTIN', fields.vendor?.gstin], ['pan', 'Vendor PAN', fields.vendor?.pan], ['buyerGstin', 'Buyer GSTIN', fields.buyer?.gstin], ...['subtotal','taxableAmount','cgst','sgst','igst','discount','roundOff','total'].map(key => [key, key, amounts[key]])];
    docs.detail.innerHTML = `<div class="detail-head"><h3>Invoice review</h3><button id="closeDoc" type="button">Close</button></div><p>${display(review.extraction_status)} · Confidence ${review.confidence_score == null ? 'Not detected' : Math.round(Number(review.confidence_score) * 100) + '%'} · ${display(review.extraction_engine)}</p><div class="document-split"><div id="originalPreview">Loading original…</div><form class="document-form" id="reviewForm"><div class="validation-warning"><ul>${(review.validation_errors || []).map(error => `<li>${escapeHtml(error)}</li>`).join('') || '<li>No validation issues detected. Verify against the original before approval.</li>'}</ul></div>${inputs.map(([key,label,value]) => `<label>${label}<input name="${key}" value="${escapeHtml(value)}" placeholder="Not detected"></label>`).join('')}<div class="line-items"><h3>Line items</h3><table><thead><tr><th>Description</th><th>HSN/SAC</th><th>Qty</th><th>Rate</th><th>Total</th></tr></thead><tbody>${(fields.items || []).map(item => `<tr><td>${display(item.description)}</td><td>${display(item.hsnSac)}</td><td>${display(item.quantity)} ${display(item.unit)}</td><td>${money(item.unitPrice)}</td><td>${money(item.lineTotal)}</td></tr>`).join('') || '<tr><td colspan="5">Not detected</td></tr>'}</tbody></table></div><label class="wide">Edit line items (JSON)<textarea name="items">${escapeHtml(JSON.stringify(fields.items || [], null, 2))}</textarea></label><label class="wide">Review notes<textarea name="notes">${escapeHtml(review.review_notes)}</textarea></label><div class="review-actions"><button value="SAVE">Save edits</button><button value="APPROVED">Approve</button><button value="REJECTED">Reject</button>${['FAILED','NEEDS_REVIEW'].includes(review.extraction_status) && !review.reviewed_at ? '<button type="button" id="retryDoc">Retry extraction</button>' : ''}</div><p id="reviewMessage" class="wide" role="status"></p></form></div><details><summary>Raw extracted text</summary><pre class="raw-text">${display(review.raw_ocr_output?.text)}</pre></details>`;
    ...
    document.getElementById('reviewForm').onsubmit = async event => {
      ...
      try {
        const items = JSON.parse(form.get('items')); if (!Array.isArray(items)) throw new Error('Line items must be a JSON array.');
        const corrected = { ...fields, invoiceNumber: form.get('invoiceNumber'), invoiceDate: form.get('invoiceDate'), vendor: { ...fields.vendor, name: form.get('vendorName'), gstin: form.get('gstin'), pan: form.get('pan') }, buyer: { ...fields.buyer, gstin: form.get('buyerGstin') }, amounts: { ...amounts }, items };
