  ...
  return {
    ...
    async cleanup() {
      ...
      await prisma.$transaction([
        ...
        prisma.payment_allocation.deleteMany({ where: { payment: { customer_id: { in: [customer.customer_id, otherCustomer.customer_id, ...created.customerIds] } } } }),
        prisma.payment.deleteMany({ where: { vendor_id: { in: [vendor.vendor_id, otherVendor.vendor_id] } } }),
        prisma.payment.deleteMany({ where: { customer_id: { in: [customer.customer_id, otherCustomer.customer_id, ...created.customerIds] } } }),
        ...
        prisma.accounts_receivable.deleteMany({ where: { sales_invoice_id: { in: created.invoiceIds } } }),
        ...
        prisma.customer.deleteMany({ where: { customer_id: { in: created.customerIds } } }),
