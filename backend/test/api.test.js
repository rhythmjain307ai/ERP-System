...
test.after(async () => {
  if (context) await require('../lib/prisma').$disconnect();
});
...
test('workflow tests require TEST_DATABASE_URL', { skip: !integration }, () => {
  assert.ok(context);
});
...
test('does not expose password hashes in user responses', { skip: !integration }, async () => {
  const response = await request(app).get('/api/master/users');
  assert.equal(response.status, 200);
  assert.ok(response.body.data.every((user) => !Object.hasOwn(user, 'password_hash')));
});
