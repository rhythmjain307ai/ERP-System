...
Open `http://localhost:4000` after starting the backend and use **Documents → Scan / Upload**. The backend accepts JPG, JPEG, PNG, and PDF files up to 25 MB by default. It saves the binary outside the source code in `backend/uploads/` (which is ignored by Git), stores only metadata in PostgreSQL, and serves originals through the authenticated `GET /api/documents/:id/file` route. The filename extension, declared MIME type, and file signature must agree. Set `OCR_MAX_UPLOAD_BYTES` to change the upload size limit.

For JPG/PNG, Tesseract.js performs local OCR; searchable PDFs use local text extraction; scanned PDF pages are rendered locally and passed through Tesseract. No cloud OCR key is required. OCR preserves page, line, word, confidence, and bounding-box data. Low-quality pages receive one adaptive grayscale/contrast retry; clean pages are not repeatedly processed. PDFs support up to 50 pages by default (`OCR_MAX_PDF_PAGES`), each OCR recognition pass has a 120-second timeout (`OCR_TIMEOUT_MS`), and two jobs run concurrently while up to 20 wait in the local queue (`OCR_MAX_CONCURRENT_JOBS`, `OCR_MAX_QUEUED_JOBS`). Limit overrides must be positive integers.

The deterministic parser classifies invoice, e-invoice, e-way bill, delivery challan, weighbridge, MRN, and goods-inward pages; maps semantic label variants; ranks invoice-number candidates using label, OCR, layout, and page evidence; and retains field-level provenance. OCR does not read invoice or receipt line items, descriptions, HSN/SAC table values, or quantities. The canonical extraction is exposed alongside compatibility fields used by the existing review UI. Validation checks GSTIN structure, conflicting identifiers, invoice-level tax totals, grand totals, and weighbridge arithmetic. Complete high-confidence extraction produces `EXTRACTED`; missing, low-confidence, contradictory, handwritten, or invalid evidence produces `UNDER_REVIEW`. OCR data is never automatically approved or posted into accounting/inventory workflows.
