...
function tokenUserId(req) {
  ...
  if (!subject || !signature) throw new ValidationError('Bearer token is invalid');
  ...
  try { return BigInt(subject); } catch { throw new ValidationError('Bearer token subject must identify a user'); }
}
