...
async function main() {
  ...
  const master = await runTransaction(async (tx) => {
    ...
    const expenseCategory = await tx.expense_category.upsert({ where: { category_name: 'Power and Fuel' }, update: { category_group: 'MANUFACTURING_CHARGES' }, create: { category_name: 'Power and Fuel', category_group: 'MANUFACTURING_CHARGES', description: 'Development expense category' } });
  ...
  await runTransaction(async (tx) => {
    ...
    const po = await tx.purchase_order.upsert({ where: { po_number: 'DEV-PO-001-PURCHASE' }, update: { vendor_id: master.vendors.vendor1.vendor_id, purchase_requisition_id: requisition.purchase_requisition_id, status: 'PARTIALLY_RECEIVED', taxable_amount: 40000, tax_amount: 7200, total_amount: 47200, expected_date: new Date('2026-09-10') }, create: { po_number: 'DEV-PO-001-PURCHASE', vendor_id: master.vendors.vendor1.vendor_id, purchase_requisition_id: requisition.purchase_requisition_id, order_date: seedDate, expected_date: new Date('2026-09-10'), taxable_amount: 40000, tax_amount: 7200, total_amount: 47200, status: 'PARTIALLY_RECEIVED', notes: 'Development purchase order' } });
    ...
    const grn = await tx.grn.upsert({ where: { grn_number: 'DEV-GRN-001' }, update: { purchase_order_id: po.purchase_order_id, vendor_id: master.vendors.vendor1.vendor_id, warehouse_id: foundation.warehouses['DEV-RMS'].warehouse_id, inspection_status: 'PASSED', remarks: 'Development goods receipt' }, create: { grn_number: 'DEV-GRN-001', purchase_order_id: po.purchase_order_id, vendor_id: master.vendors.vendor1.vendor_id, warehouse_id: foundation.warehouses['DEV-RMS'].warehouse_id, grn_date: seedDate, inspection_status: 'PASSED', inspected_by: access.users['Stores Manager'].user_id, remarks: 'Development goods receipt' } });
