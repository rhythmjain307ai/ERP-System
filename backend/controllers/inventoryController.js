...
exports.updateStock = async (req, res, next) => {
  try {
    ...
    const sku = await prisma.inventory.findUnique({ where: { id: skuId } });
    if (!sku) return res.status(404).json({ error: 'SKU not found' });
    if (sku.stock + Number(quantityChange) < 0) return res.status(400).json({ error: 'Insufficient stock' });
    const updated = await prisma.inventory.update({ where: { id: skuId }, data: { stock: sku.stock + Number(quantityChange) } });
    res.json(updated);
  } catch (err) { next(err); }
};
